<template>
	<q-dialog v-model="isOpen" persistent class="container-modal" transition-show="none" transition-hide="none"
		backdrop-filter="blur(4px) saturate(150%)" :fullscreen="isMobile">
		<div class="form-modal-view div-blur div-rounded-radius" :style="dialogStyle">
			<q-card flat class="bg-blur">
				<q-card-section class="q-pa-none">
					<div class="row justify-center items-center column q-py-xs">
						...
						<q-item-section class="q-pb-md text-center">
			...
			<div class="row q-mx-none q-gutter-x-md" style="display: flex; flex-wrap: wrap;">
				<q-card v-for="route in tableData" :key="route.route_id" class="bg-first div-rounded-radius" flat
					:class="isMobile ? 'col-12' : (tableData.length > 1 ? 'col-md-6' : 'col-12')" :style="{
						margin: isMobile ? '10px 0' : '10px', // Ajuste de margen para escritorio
						flex: isMobile ? '1 1 100%' : '1 1 calc(50% - 20px)' // Ajustar el tamaño para mantener dos tarjetas por fila en escritorio
					}">
					<q-card-section class="q-pa-none">
...
const dialogStyle = computed(() => {
	return isMobile.value ? 'width: 100vw; max-width: 100vw; height: 100vh !important;margin: 8px;' : 'width: 900px; max-width: 100vw;';
});
const filter = ref('');
const tableData = ref([]);
